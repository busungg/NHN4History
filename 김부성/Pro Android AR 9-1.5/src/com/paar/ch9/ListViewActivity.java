...
public class ListViewActivity extends Activity {
	...
	// 데이터를 담을 자료구조
	private ArrayList<CData> alist = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int num,
				...
				String id = collection.get(num).getId();
				
				LocationView.marker_id = ((id.length() == 9) ? id.substring(1, 9) : id.substring(2, 10));
				LocationView.check_icon = NaverDataSource.check_icon;
				Intent intent = new Intent(ListViewActivity.this, LocationView.class);
	...
	private void AddData(String text)
	{
		...
		if(text == null)
		{
			for (Marker marker : collection) {
				adapter.add(new CData(getApplicationContext(), marker.getName(),
						FORMAT.format(marker.getDistance())+ " m", reicon()));
			}
		}
	...
	private class DataAdapter extends ArrayAdapter<CData> {
		...
		public View getView(int position, View v, ViewGroup parent) {
			...
			if (data != null) {
				// 화면 출력
				TextView title = (TextView) view.findViewById(R.id.item_row_title);
				TextView description = (TextView) view.findViewById(R.id.item_row_description);
				...
				//이미지 뷰
				ImageView iv = (ImageView) view.findViewById(R.id.item_row_draw);
