...
String mapView = (String)request.getAttribute("mapView");

String webKey = "b60fa1bbda6024ee3cd76d495c6f82d4";
String mobileKey = "2b3d1326fad8b535bd8320e3b101e6da";
String mapKey = "";

System.out.println(request.getAttribute("mapView"));

if(request.getAttribute("mapView").equals("mobile"))
{
	mapKey = mobileKey;
}
else
...
<script>
	...
	function deletePost(){
		...
		document.location.href="/memory/Service?action=deletePost&postId=" + postId + "&startDate=" + startDate + "&endDate=" + endDate+"&mapView="+ mapView;
	}
	...
	function plusPost(){
		...
		location.href="/memory/pages/write.jsp?date=" + startDate;
	}
</script>
...
<body>
	...
	<div id="all" align="center">
		...
		<div id="tabs"
			...
			<%=data.getText()%>
			<%=data.getUptime() + " "+ data.getShopname() +" " + data.getCategory()%>
			<%="</p> </div>"%>
			<%
				}
			%>
		...
		<br>
		<div>
			<input type="button" class="large button blue" style="width:80px" value="Delete" onclick="deletePost();">
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" class="large button blue" style="width:80px" value="Add" onclick="plusPost();">
		</div>
		<!-- 맵부분 -->
		<div id="map"
			...
			<p style="color: white;">
			<%if(mapKey.equals(mobileKey)){ 
				System.out.println(mobileKey);
			%>	
					document.write("<script type='text/javascript' src='http://openapi.map.naver.com/openapi/naverMap.naver?ver=2.0&key=2b3d1326fad8b535bd8320e3b101e6da'></script>");
			<%-- <script type="text/javascript"
				src="http://openapi.map.naver.com/openapi/naverMap.naver?ver=2.0&key=<%=mapKey%>">
			</script> --%>
			<%}else{ %>
					document.write("<script type='text/javascript' src='http://openapi.map.naver.com/openapi/naverMap.naver?ver=2.0&key=b60fa1bbda6024ee3cd76d495c6f82d4'></script>");
			<%}%>
			</p>
		...
		<div id="share" align="center" style="margin: 10px;">
			...
			<form  action="/memory/Service?action=postshare" method="post">
				<button type="submit" class="large button blue" style="width:80px"/>
				Share
				</button>
